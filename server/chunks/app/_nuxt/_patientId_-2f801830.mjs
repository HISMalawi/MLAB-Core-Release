...
import { _ as __nuxt_component_1 } from './index-cf647ce1.mjs';
import { _ as _sfc_main$2 } from './Address-f3c26c99.mjs';
import { _ as __nuxt_component_3 } from './Loader-86943425.mjs';
import { a as dateRange, d as dateFormat } from './constants-353d90a1.mjs';
import { e as endpoints, f as fetchRequest, d as calculateAge } from './fetch-bdf4b52b.mjs';
...
const _sfc_main = {
  ...
  methods: {
    ...
    checkSusceptibilityResults(report) {
      return report.tests.some((test) => test.suscept_test_result.length !== 0);
    },
    ...
    shouldPrintSmallLabel(orders) {
      return orders.some((order) => order.tests.some((test) => test.print_device));
    },
    ...
    getPrintCount(reportData) {
      return reportData.reduce((count, report) => count + report.print_count, 0);
    },
    getRequiredRange(patient, ranges) {
      ...
      return ranges.filter((range) => range.sex === patientSex && age > Number(range.min_age) && age < Number(range.max_age) || range.sex === "Both" && age > Number(range.min_age) && age < Number(range.max_age))[0];
    },
    ...
    ssrRenderList($data.reportData, (report, index) => {
      ...
      ssrRenderList($data.showPendingTests ? report.tests : report.tests.filter((item) => item.status.toLowerCase() !== "pending"), (test) => {
        ...
        ssrRenderList(test.indicators, (indicator) => {
          ...
          _push(`</td>`);
          if ($options.shouldDisplayRange(test.indicators, $data.person, indicator) !== void 0 && $options.shouldDisplayRange(test.indicators, $data.person, indicator) !== null) {
...
//# sourceMappingURL=_patientId_-2f801830.mjs.map

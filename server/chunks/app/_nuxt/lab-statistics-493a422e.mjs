  ...
  setup(__props) {
    ...
    async function generateReport() {
      ...
      if (data.value) {
        ...
        loading.value = false;
        data.value.data.length > 0 ? useNuxtApp().$toast.success("Report data generated successfully") : useNuxtApp().$toast.warning(`No data found for period ${startDate} - ${endDate}`);
      }
      if (error.value) {
        loading.value = false;
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(_component_excel, {
        ...
        header: [`LABORATORY STATISTICS REPORT `, `PERIOD FROM ${unref(moment)(unref(dateRange)[0].toString()).format("DD-MM-yyyy")} TO ${unref(moment)(unref(dateRange)[1].toString()).format("DD-MM-yyyy")}`, unref(facility).details.name, unref(facility).details.address, unref(facility).details.phone],
        data: unref(exportData),
        worksheet: "report-work-sheet",
        name: `lab-statistics-report_${unref(moment)(unref(dateRange)[0].toString()).format("DD_MM_yyyy")}_to_${unref(moment)(unref(dateRange)[1].toString()).format("DD_MM_yyyy")}.xls`
      }, {
...
//# sourceMappingURL=lab-statistics-493a422e.mjs.map

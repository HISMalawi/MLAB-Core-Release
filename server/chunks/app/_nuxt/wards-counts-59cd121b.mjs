...
import { _ as _sfc_main$2 } from './Address-f3c26c99.mjs';
...
import { e as endpoints, f as fetchRequest } from './fetch-bdf4b52b.mjs';
  ...
  setup(__props) {
    ...
    const exportData = computed(() => {
      return headers.value.flatMap((header) => {
        return reportData.value.map((report) => ({
          "WARD": report.ward,
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(_component_excel, {
        ...
        header: [`CULTURE & SENSITIVITY WARDS COUNTS REPORT ${unref(moment)(unref(dateFrom)).format("M/yyyy")}`, unref(facility).details.name, unref(facility).details.address, unref(facility).details.phone],
        data: unref(exportData),
        worksheet: "report-work-sheet",
        name: `culture_sensitivity_wards_counts_report_${unref(moment)(unref(dateFrom)).format("M_yyyy")}.xls`
      }, {
      ...
      _push(`</div></div><div class="rounded border"><div class="rounded-tr rounded-tl border-b px-5 py-5 flex items-center justify-between"><div class="flex flex-col space-y-2"><img${ssrRenderAttr("src", _imports_0)} alt="app-logo" class="w-24 h-24 object-cover"><h3 class="text-xl font-semibold">CULTURE &amp; SENSITIVITY WARDS COUNTS REPORT </h3></div>`);
      _push(ssrRenderComponent(_component_ReportsAddress, null, null, _parent));
      _push(`</div><div><h3 class="px-4 mt-2 font-medium">Data for period: <span class="font-normal">${ssrInterpolate(unref(dateFrom) == "" ? "" : unref(moment)(unref(dateFrom)).format("M/yyyy"))}</span></h3></div><div id="print-container"><table class="w-full mt-3"><thead class="w-full border-b border-t bg-gray-50 rounded-t"><tr class="w-full"><!--[-->`);
      ...
      ssrRenderList(unref(reportData), (report, index) => {
        _push(`<tr class="border-b border-l border-r"><td class="px-2 py-2 border-r">${ssrInterpolate(report.ward)}</td><td class="px-2 py-2 border-r">${ssrInterpolate(report.encounter)}</td><td class="px-2 py-2 border-r">${ssrInterpolate(unref(moment)(unref(dateFrom)).format("MMMM-yyyy"))}</td><td class="px-2 py-2">${ssrInterpolate(report.count)}</td></tr>`);
      });
...
//# sourceMappingURL=wards-counts-59cd121b.mjs.map

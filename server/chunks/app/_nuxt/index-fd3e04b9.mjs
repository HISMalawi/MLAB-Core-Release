...
import { _ as _export_sfc, a as useCookie, b as useNuxtApp, d as __nuxt_component_0$1 } from '../server.mjs';
import { resolveComponent, withCtx, createVNode, withDirectives, vModelCheckbox, openBlock, createBlock, vModelText, createCommentVNode, useSSRContext } from 'vue';
import { useRoute } from 'vue-router';
import { u as useAuthStore, o as useAlert, d as calculateAge, e as endpoints, f as fetchRequest, p as isEmpty, P as PrinterService, A as Api } from './fetch-40f40580.mjs';
...
import { ssrRenderAttrs, ssrRenderComponent, ssrInterpolate, ssrIncludeBooleanAttr, ssrLooseContain, ssrRenderAttr } from 'vue/server-renderer';
...
const _sfc_main = {
  ...
  computed: {
    ...
    currentDepartment() {
      return this.authStore.user.departments.find((item) => item.name === this.authStore.department);
    }
  },
  methods: {
    ...
    async saveChanges() {
      ...
      orderService.createEncounter(this.visitTypeSelected.id, this.wardSelected.id);
      ...
      if (!isEmpty(order)) {
        useNuxtApp().$toast.success(`Order with accession number ${order.accession_number} has been created successfully!`);
        if (await this.alert.alertConfirmation({ message: "Do you want to print specimen label?" })) {
  ...
  watch: {
    async visitTypeSelected(visitType) {
      ...
      if (isEmpty(this.wards[visitType.id])) {
        this.wards = await Api.getJson("encounter_type_facility_section_mappings/facility_sections", {
    ...
    async authStore(authStore) {
      ...
      if (!isEmpty(this.specimenSelected)) {
        this.tests[this.specimenSelected.id] = await Api.getJson("specimen/test_types", {
          ...
          department_id: authStore.user.departments.find((item) => item.name === authStore.department).id
        }) || [];
      }
    }
  },
...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  ...
  _push(`<div class="mt-5"><h3 class="text-xl font-medium">New test for ${ssrInterpolate($options.patientName)}</h3><div class="border rounded mt-5"><div class="bg-gray-50 flex items-center space-x-3 px-2 py-2 rounded-tr rounded-tl border-b">`);
  ...
  _push(ssrRenderComponent(_component_FormKit, {
    ...
    default: withCtx(({ value }, _push2, _parent2, _scopeId) => {
      if (_push2) {
        ...
        _push2(`</div><div class="w-1/2 flex flex-col space-y-2"${_scopeId}><label class="font-medium"${_scopeId}>Requesting Ward</label>`);
        _push2(ssrRenderComponent(_component_CoreDropdown, {
          items: $data.wards,
        ...
        return [
          createVNode("div", { class: "border px-5 py-5 mt-5 rounded" }, [
            ...
            createVNode("div", { class: "w-full flex items-center space-x-10 mb-5" }, [
              ...
              createVNode("div", { class: "w-1/2 flex flex-col space-y-2" }, [
                createVNode("label", { class: "font-medium" }, "Requesting Ward"),
                createVNode(_component_CoreDropdown, {
                  items: $data.wards,
                  modelValue: $data.wardSelected,
                  "onUpdate:modelValue": ($event) => $data.wardSelected = $event
...
//# sourceMappingURL=index-fd3e04b9.mjs.map

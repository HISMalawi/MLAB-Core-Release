...
import { u as useCookie, a as useNuxtApp, b as __nuxt_component_0$1, _ as _export_sfc } from '../server.mjs';
import { _ as _sfc_main$5 } from './Address-f3c26c99.mjs';
  ...
  setup(__props) {
    ...
    async function generateReport() {
      ...
      if (reportType.value.value != "select report type") {
        let queryParams = `from=${startDate.value}&to=${endDate.value}&department=${checkStatus(selectedDepartment.value.name)}&report_type=${reportType.value.value}&test_status=${checkStatus(selectedTestStatus.value.name)}&test_type=${checkStatus(selectedtTestType.value.name)}`;
    ...
    function calculateCompleteTests(reportData2) {
      reportData2.data.map((report2) => {
        if (report2.test_status.toLowerCase() === "completed" || report2.test_status.toLowerCase() === "verified") {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      const _component_ReportsAddress = _sfc_main$5;
      const _component_DailyLogTestRecords = __nuxt_component_4;
      const _component_DailyLogPatientRecords = __nuxt_component_5;
      const _component_DailyLogSpecimenRejected = __nuxt_component_6;
      ...
      _push(ssrRenderComponent(unref(ExportToExcel), {
        ...
        filename: `daily_${unref(reportType).value}_${unref(moment)(/* @__PURE__ */ new Date()).format("yyy_MM_DD")}`
      }, {
...
//# sourceMappingURL=daily-log-23b4827a.mjs.map

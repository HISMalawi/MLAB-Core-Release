  ...
  if ($data.reportData.length > 0) {
    ...
    ssrRenderList($data.reportData, (report, index) => {
      ...
      ssrRenderList($data.showPendingTests ? report.tests : report.tests.filter((item) => item.status.toLowerCase() !== "pending"), (test) => {
        var _a;
        _push(`<tr><td class="border px-2 py-2">${ssrInterpolate(test.test_type_name)}</td><td class="border"><table class="w-full h-full"><thead><tr><th class="border-r border-b px-2 py-2 font-bold"> Measure </th><th class="border-r border-b px-2 py-2 font-bold"> Result </th><th class="px-2 py-2 border-b font-bold">Range</th></tr></thead><!--[-->`);
        ssrRenderList(test.indicators, (indicator) => {
          var _a3;
          var _a2, _b;
          _push(`<tr><td class="border-r border-b px-2 py-2">${ssrInterpolate(indicator.name)}</td><td class="border-b px-2 py-2 flex items-center"><p>${$options.showResult(
        ...
        if ($options.getTestStatusInitiatorName(
          ...
          _push(`<div><p class="font-bold">Performed By</p><p class="px-2">${ssrInterpolate($options.getTestStatusInitiatorName(
            test.status_trail,
            "completed"
          )["name"])}</p><p class="px-2 mb-2"> On: ${ssrInterpolate($data.moment(
            $options.getTestStatusInitiatorName(
              test.status_trail,
              "completed"
            )["created_at"]
          ).format("dateFormat" in _ctx ? _ctx.dateFormat : unref(dateFormat)))}</p>`);
          if ($options.getMachineName(test.indicators)) {
            _push(`<div class="mb-2"><h3 class="font-bold">Laboratory Machine Used</h3><p>${ssrInterpolate($options.getMachineName(test.indicators))}</p></div>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</div>`);
...
//# sourceMappingURL=_patientId_-1a1e51e3.mjs.map

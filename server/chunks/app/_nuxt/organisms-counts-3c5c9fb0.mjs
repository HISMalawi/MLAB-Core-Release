  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(_component_excel, {
        ...
        header: [`CULTURE & SENSITIVITY ORGANISMS COUNTS REPORT ${unref(moment)(unref(dateFrom)).format("M/yyyy")}`, unref(facility).details.name, unref(facility).details.address, unref(facility).details.phone],
        data: unref(exportData),
        worksheet: "report-work-sheet",
        name: `culture_sensitivity_organisms_counts_report_${unref(moment)(unref(dateFrom)).format("M_yyyy")}.xls`
      }, {
      ...
      ssrRenderList(unref(reportData), (report, index) => {
        _push(`<tr class="border-b"><td class="px-2 py-2 text-left border-r">${ssrInterpolate(report.organism)}</td><td class="px-2 py-2 text-left border-r">${ssrInterpolate(unref(moment)(unref(dateFrom)).format("MMMM/yyyy"))}</td><td class="px-2 py-2 text-left">${ssrInterpolate(report.count)}</td></tr>`);
      });
...
//# sourceMappingURL=organisms-counts-3c5c9fb0.mjs.map
